...
const CheckoutPage = () => {
  ...
  const total = Cart.reduce((acc,item)=> acc= acc+item.price*item.quantity,0)
  useEffect(()=> {
      ...
      <div className="w-full max-w-4xl flex flex-col md:flex-row gap-6">
        <div className="w-full md:w-2/3 bg-white p-6 rounded shadow-md">
          ...
          <form onSubmit={handleSubmit} className="grid gap-4">
           ...
           {! address ? ( <Button type="submit" className="w-full mt-4">
              {loading ? 'Loading...' : 'Add Address'}
            </Button>):(<Button  className="w-full mt-4" >
